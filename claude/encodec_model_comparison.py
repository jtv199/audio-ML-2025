#!/usr/bin/env python3
"""
Model Comparison Script for EnCodec Token Embeddings
Trains and evaluates 24+ sklearn classifiers on EnCodec tokenized audio features.
"""
...
def load_and_prepare_data():
    """Load EnCodec tokenized features and labels, prepare train/test split"""
    ...
    print(f"  Loading tokenized features from {FEATURES_CSV}")
    features_df = pd.read_csv(FEATURES_CSV)
    print(f"  Tokenized features shape: {features_df.shape}")

    # The tokenized CSV already has labels, so we can use it directly
    # Extract labels and metadata
    df = features_df.copy()

    print(f"  Total samples: {len(df)}")

    # Filter to single-class samples (for classification)
...
def save_results(all_results, output_dir='./claude'):
    ...
    all_results_file = output_path / f'encodec_model_comparison_results_{timestamp}.csv'
    ...
    if len(successful) > 0:
        successful_file = output_path / f'encodec_model_comparison_successful_{timestamp}.csv'
        ...
        top_10_file = output_path / f'encodec_model_comparison_top10_{timestamp}.csv'
...
def main():
    ...
    print("=" * 80)
    print("SKLEARN MODEL COMPARISON - ENCODEC TOKENIZED AUDIO")
    print("=" * 80)
