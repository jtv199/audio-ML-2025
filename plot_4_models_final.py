#!/usr/bin/env python3
"""
Plot sample size vs accuracy for 4 models with REAL per-class data:
- LinearSVC (baseline)
- MobileNetV4 (CNN)
...
# DeiT Tiny - NOW WITH PER-CLASS DATA!
deit_df = pd.read_csv('claude/lazypredict/findings/error_analysis_deit_tiny.csv')
print(f"DeiT Tiny: {len(deit_df)} classes, Mean accuracy: {deit_df['Accuracy'].mean():.2%}")
...
df = df.merge(
    deit_df[['Class', 'Accuracy']].rename(columns={'Accuracy': 'Accuracy_deit'}),
    on='Class',
    how='outer'
)
...
ax.scatter(df['Total_Samples'], df['Accuracy_deit'],
          s=size_scatter, alpha=alpha_scatter, c=colors['DeiT Tiny'],
          label='DeiT Tiny (10.67%)', edgecolors='white', linewidth=0.5)
...
# DeiT Tiny trend
valid_deit = df['Accuracy_deit'] > 0
z_deit = np.polyfit(df.loc[valid_deit, 'Total_Samples'], df.loc[valid_deit, 'Accuracy_deit'], 2)
p_deit = np.poly1d(z_deit)
ax.plot(x_range, p_deit(x_range), color=colors['DeiT Tiny'], linewidth=3,
        alpha=0.8, linestyle='-')
...
corr_deit = stats.spearmanr(df.loc[valid_deit, 'Total_Samples'], df.loc[valid_deit, 'Accuracy_deit'])[0]
...
print(f"  DeiT Tiny:        {corr_deit:.3f}")
...
mean_deit = df.loc[valid_deit, 'Accuracy_deit'].mean()
...
ax.axhline(y=mean_deit, color=colors['DeiT Tiny'], linestyle=':', alpha=0.3, linewidth=1)
...
ax.set_title('Model Comparison: Sample Size vs Accuracy (4 Models)\nLinearSVC vs MobileNetV4 vs DeiT Tiny vs VGGish+Tabular',
...
legend_labels = [
    ...
    f'DeiT Tiny (10.67% avg, corr={corr_deit:.3f})',
...
textstr = '\n'.join([
    ...
    '• DeiT Tiny: Failed (10.67%)',
    '',
    'Sample Dependency:',
    f'• VGGish+Tab: {corr_vgg_tab:.3f} (moderate)',
    f'• DeiT: {corr_deit:.3f} (weak)',
    f'• LinearSVC: {corr_svc:.3f} (weak)',
    f'• MobileNet: {corr_mob:.3f} (none)',
])
...
# Save figure
output_file = 'comparison_4_models_final.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"\n✓ Plot saved to: {output_file}")

# Also save the merged data
df.to_csv('comparison_4_models_final.csv', index=False)
print(f"✓ Data saved to: comparison_4_models_final.csv")
...
print(f"  DeiT Tiny:        {mean_deit:.2f}% ± {df.loc[valid_deit, 'Accuracy_deit'].std():.2f}%")
...
print(f"  DeiT Tiny:        {len(df[df['Accuracy_deit'] == 100])}")
...
print(f"  DeiT Tiny:        {len(df[df['Accuracy_deit'] == 0])}")
