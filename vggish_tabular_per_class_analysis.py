#!/usr/bin/env python3
"""
VGGish + Tabular Per-Class Error Analysis
Properly evaluate the model on each class separately
"""
...
print("🚀 Starting VGGish+Tabular Per-Class Error Analysis\n")
...
# Prepare validation dataset (20% split, same seed as training)
...
feature_cols = [col for col in val_df.columns if col not in ['fname', 'labels', 'label', 'label_count']]
print(f"Feature columns: {len(feature_cols)}")

# Make predictions using get_preds on a test dataloader
print("\n🔮 Making predictions on validation set...")

# Create a TabularDataLoaders from the validation set
from fastai.tabular.all import TabularDataLoaders, Categorify, FillMissing, Normalize

# Get categorical and continuous columns from the learner
cat_names = learn.dls.cat_names if hasattr(learn.dls, 'cat_names') else []
cont_names = learn.dls.cont_names if hasattr(learn.dls, 'cont_names') else feature_cols
y_names = 'label'

# Create test dataloader
test_dl = learn.dls.test_dl(val_df[feature_cols + ['label']])
...
y_pred = [learn.dls.vocab[i] for i in preds.argmax(dim=1)]
...
print("\n" + "="*70)
print("VGGISH+TABULAR PER-CLASS ERROR ANALYSIS")
print("="*70)
...
try:
    ...
    print("\n📊 VGGish vs MobileNet (Top 10 where VGGish wins):")
    comp_mob = comparison[comparison['Improvement_vs_MobileNet'] > 0].nlargest(10, 'Improvement_vs_MobileNet')
    if len(comp_mob) > 0:
        ...
        print("  MobileNet wins on all classes!")
