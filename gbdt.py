...
class LightGBM():
    ...
    def train(
        ...
        for i in pbar:
            ...
            pred = bst.predict(val_X) > threshold
    ...
    def pred(self, X):
        length = len(X)
        probs = np.zeros((length, self.n_labels))
        preds = np.zeros((length, self.n_labels))
        for i in range(self.n_labels):
            proba = self.bsts[i].predict(X)
            pred = proba > self.thresh[i]
            probs[:,i] = proba
            preds[:,i] = pred
        return probs, preds
