#!/usr/bin/env python3
"""
MobileNetV4 Error Analysis
Load the trained MobileNet model and perform comprehensive error analysis
Similar to LinearSVC analysis but for deep learning model
"""
...
print("🚀 Starting MobileNetV4 Error Analysis\n")
...
MODEL_PATH = Path('models/mobilenetv4_conv_small.pkl')

# Check if model exists
if not MODEL_PATH.exists():
    print(f"❌ Model not found at {MODEL_PATH}")
    print("Looking for alternative models...")
    alternatives = list(Path('models').glob('*.pkl'))
    if alternatives:
...
# Save results
output_file = 'claude/lazypredict/findings/error_analysis_mobilenet.csv'
results_df.to_csv(output_file, index=False)
print(f"\n✓ Results saved to: {output_file}")

# Print summary statistics
print("\n" + "="*70)
print("MOBILENET ERROR ANALYSIS SUMMARY")
print("="*70)
...
print("\n" + "="*70)
print("COMPARISON WITH LINEARSVC")
print("="*70)
...
try:
    ...
    comparison = results_df[['Class', 'Accuracy']].merge(
        ...
        on='Class',
        suffixes=('_mobilenet', '_linearsvc'),
        how='inner'
    )
    comparison['Improvement'] = (comparison['Accuracy_mobilenet'] - comparison['Accuracy_linearsvc']) * 100
    comparison = comparison.sort_values('Improvement', ascending=False)

    print(f"\nMobileNet Overall: {accuracy*100:.2f}%")
    print(f"LinearSVC Overall: {linearsvc_df['Accuracy'].mean()*100:.2f}%")
    print(f"Difference: {(accuracy - linearsvc_df['Accuracy'].mean())*100:+.2f}%")

    print("\n📈 Classes with BIGGEST IMPROVEMENTS:")
    for _, row in comparison.head(10).iterrows():
        print(f"  {row['Class']:40s} {row['Improvement']:+6.1f}% "
              f"({row['Accuracy_mobilenet']*100:.1f}% vs {row['Accuracy_linearsvc']*100:.1f}%)")

    print("\n📉 Classes with BIGGEST REGRESSIONS:")
    for _, row in comparison.tail(10).iterrows():
        print(f"  {row['Class']:40s} {row['Improvement']:+6.1f}% "
              f"({row['Accuracy_mobilenet']*100:.1f}% vs {row['Accuracy_linearsvc']*100:.1f}%)")

    # Save comparison
    comparison.to_csv('claude/lazypredict/findings/mobilenet_vs_linearsvc_comparison.csv', index=False)
    print(f"\n✓ Comparison saved to: claude/lazypredict/findings/mobilenet_vs_linearsvc_comparison.csv")

except Exception as e:
    print(f"  Could not compare with LinearSVC: {e}")
